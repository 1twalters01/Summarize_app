...
use crate::{
    datatypes::{email_types::{MessageType::RegisterEmail, RegisterEmailParams}, response_types::{AppError, AppResponse}},
...
pub async fn post_email(data: ProtoBuf<Request>) -> Result<impl Responder> {
    ...
    let _ = match user_service.get_user_uuid_from_email(&email).await {
        Ok(Some(_)) => {
